...
<title>LIA - Reconstrução de baixo para cima</title>
<style>
html,body{
  margin:0;
  width:100%;
...
.scan{
  ...
  bottom:4%;
  opacity:0;
  background:linear-gradient(90deg,transparent,#9efcff,#00d9ff,#9efcff,transparent);
  box-shadow:0 0 10px #00d9ff,0 0 24px #00d9ff;
  animation:scanUp 2.55s linear .55s forwards;
}
@keyframes rebuild{
...
<div class="stage">
  <div class="lia-wrap">
   <video id="lia" preload="auto" playsinline><source src="https://appassets.local/leal_ai_feminino_transparente.webm" type="video/webm"></video>
    <canvas id="fx"></canvas>
    <div class="scan"></div>
  </div>
...
const wrap=document.querySelector('.lia-wrap');
...
for(let i=0;i<N;i++){
  particles.push({
    x:Math.random(),
    y:0.92+Math.random()*0.10,
    tx:0.18+Math.random()*0.64,
    ty:0.04+Math.random()*0.92,
    size:0.7+Math.random()*1.8,
    delay:Math.random()*1600,
    life:1000+Math.random()*1100,
    drift:(Math.random()-.5)*0.08
  });
}
...
function animate(now){
  ...
  for(const p of particles){
    const local=t-350-p.delay;
    if(local<0 || local>p.life) continue;
    const q=Math.min(1,local/p.life);
    const ease=1-Math.pow(1-q,3);
    const x=(p.x+(p.tx-p.x)*ease + Math.sin(q*Math.PI)*p.drift)*r.width;
    const y=(p.y+(p.ty-p.y)*ease)*r.height;
    const a=Math.sin(Math.PI*q);
    ctx.globalAlpha=a*0.85;
    ctx.fillStyle = (Math.random()>.72) ? '#d8ffff' : '#36e8ff';
  ...
  ctx.globalAlpha=1;
  ctx.shadowBlur=0;
  if(t<3700) requestAnimationFrame(animate);
}
requestAnimationFrame(animate);

// Mantém o primeiro frame durante a materialização e inicia a fala quando a LIA já apareceu.
const lia=document.getElementById('lia');
lia.addEventListener('loadedmetadata',()=>{
  try{ lia.currentTime=0; }catch(e){}
});
setTimeout(()=>{
  lia.play().catch(()=>{});
},3250);
...
holo.CoreWebView2.Settings.AreDefaultContextMenusEnabled = false;
            ...
            holo.CoreWebView2.SetVirtualHostNameToFolderMapping(
                "appassets.local",
