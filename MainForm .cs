...
public sealed class MainForm : Form
{
    ...
    private async void ShowLealAiPanel(bool dailyGreeting)
    {
        ...
        string imageData;
        try { imageData = Convert.ToBase64String(File.ReadAllBytes(imagePath)); }
        catch (Exception ex)
        {
            MessageBox.Show("Não foi possível carregar a imagem da LIA.\n\n" + ex.Message, "LEAL AI");
            return;
        }
...
#core{position:absolute;left:43%;right:43%;bottom:3.6%;height:3.5%;border-radius:50%;background:#eaffff;box-shadow:0 0 10px white,0 0 28px #00e5ff,0 0 55px #008cff;animation:corePulse 1.05s ease-in-out infinite}
.p{position:absolute;width:3px;height:3px;border-radius:50%;background:#b8fbff;box-shadow:0 0 7px #00dfff;opacity:0;animation:particle 3s linear infinite}
@keyframes flicker{0%,18%,22%,24%,53%,55%,100%{opacity:.70;transform:translateX(0) scaleY(1)}19%{opacity:.47;transform:translateX(2px)}23%{opacity:.80;transform:translateX(-2px)}54%{opacity:.57;transform:translateX(1px) scaleY(.997)}}
@keyframes beam{50%{opacity:.58;filter:blur(12px)}}@keyframes scan{to{background-position:0 45px}}
@keyframes glitch{0%,92%,100%{opacity:0;transform:translateX(0)}93%{opacity:.72;transform:translateX(-13px)}94%{opacity:.28;transform:translateX(16px)}95%{opacity:0}}
@keyframes basePulse{0%,100%{opacity:.95;transform:perspective(180px) rotateX(66deg) scale(1)}50%{opacity:.62;transform:perspective(180px) rotateX(66deg) scale(.94)}}
@keyframes corePulse{50%{opacity:.55;transform:scale(.84)}}@keyframes particle{0%{opacity:0;transform:translateY(0)}18%{opacity:.9}100%{opacity:0;transform:translateY(-290px)}}
</style></head><body><div id="stage"><div id="beam"></div><img id="lia" src="data:image/png;base64,__LIA_IMAGE__"><div class="scan"></div><div class="glitch" id="g1"></div><div class="glitch" id="g2"></div><div class="glitch" id="g3"></div><div id="base"></div><div id="core"></div></div>
<script>const s=document.getElementById('stage');for(let i=0;i<38;i++){const p=document.createElement('i');p.className='p';p.style.left=(16+Math.random()*68)+'%';p.style.bottom=(5+Math.random()*30)+'%';p.style.animationDelay=(Math.random()*3)+'s';p.style.animationDuration=(2+Math.random()*2.8)+'s';s.appendChild(p);}</script>
</body></html>
""".Replace("__LIA_IMAGE__", imageData);

        var holo = new WebView2
        {
            Width = 345,
            Height = 570,
            BackColor = Color.Transparent,
        ...
        void PosHologram()
        {
            ...
            holo.Left = Math.Max(8, host.ClientSize.Width - holo.Width - 18);
            holo.Top = Math.Max(8, host.ClientSize.Height - holo.Height - 4);
            holo.BringToFront();
            lealAiButton?.BringToFront();
        }
        ...
        holo.Disposed += (_, _) => { if (resizeHandler != null) host.Resize -= resizeHandler; };
        ...
        catch (Exception ex)
        {
            if (!holo.IsDisposed)
            {
                host.Controls.Remove(holo);
                holo.Dispose();
            }
            lealAiHologram = null;
            MessageBox.Show("Não foi possível iniciar o holograma WebView2.\n\n" + ex.Message, "LEAL AI", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
    }
